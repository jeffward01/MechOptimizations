'use strict';
app.controller('licensesMechanicalController', ['$scope', 'licensesService', 'licenseProductsService', 'contactsService', '$modal', '$stateParams', 'localStorageService', 'licenseesService', 'labelsService', 'prioritiesService', 'contactDefaultsService', 'licenseMethodsService', 'rateTypesService', '$filter', '$state', 'safeService', 'reportsService', 'notyService', function ($scope, licensesService, licenseProductsService, contactsService, $modal, $stateParams, localStorageService, licenseesService, labelsService, prioritiesService, contactDefaultsService, licenseMethodsService, rateTypesService, $filter, $state, safeService, reportsService, notyService) {
    ...
    $scope.reportRequest = {
        SearchTerm: "",
        SearchInRecordingTitle: null,
        SearchInProductTitle: null,
        SearchInLicenseTitle: null,
        SearchInArtist: null,
        SearchInWriter: null,
        SearchInPips: null,
        SearchInUPC: null,
        SearchInClientCode: null,
        RecsLabelGroupIds: null,
        RecsLabelIds: null,
        RecsPublisherIds: null,
        RecsConfigurationsIds: null,
        RateTypeIds: null,
        LicenseMethodIds: null,
        AssigneeContactIds: null,
        LicenseTypeIds: null,
        LicenseStatusIds: null,
        LicenseProductStatusReport: null,
        ProductPriorityReport: null,
        Status: null,
        ReleaseDateBegin: null,
        ReleaseDateEnd: null,
        SignedDateBegin: null,
        SignedDateEnd: null,
        ReportTypeId: 2,
        Notes: null,
        CreatedBy: $scope.safeauthentication != null ? $scope.safeauthentication.contactId : null,
    ...
    $scope.selectAllLicenses = function () {
        var allSelected = !$filter('isAllSelected')($scope.licenses);
        $filter('selectAll')($scope.licenses, allSelected);
    ...
    $scope.searchParameters = function () {
        ...
        return {
            ...
            WritersConsentTypes: USL.Common.SingleFieldArray(advancedOpt.WritersConsentTypes, 'writersConsentTypeId', true),
    ...
    $scope.check = function (isCollapsed, liceseId) {
        if (isCollapsed) {
    ...
    //$scope.initialSearch();
     ...
     $scope.sendReport = function () {
         $scope.reportRequest.SearchTerm = $scope.searchParameters().SearchTerm;
         $scope.reportRequest.SearchInRecordingTitle = $scope.searchParameters().IncludeRecordingTitle;
         $scope.reportRequest.SearchInProductTitle = $scope.searchParameters().IncludeProductTitle;
         $scope.reportRequest.SearchInLicenseTitle = $scope.searchParameters().IncludeLicenseTitle;
         $scope.reportRequest.SearchInArtist = $scope.searchParameters().IncludeArtist;
         $scope.reportRequest.SearchInWriter = $scope.searchParameters().IncludeWriter;
         $scope.reportRequest.SearchInPips = $scope.searchParameters().IncludePipsCode;
         $scope.reportRequest.SearchInUPC = $scope.searchParameters().IncludeUpcCode;
         $scope.reportRequest.SearchInClientCode = $scope.searchParameters().IncludeClientCode;
         $scope.reportRequest.RecsLabelGroupIds = null;
         $scope.reportRequest.RecsLabelIds = $scope.searchParameters().Labels.toString();
         $scope.reportRequest.RecsPublisherIds = $scope.searchParameters().Publishers.toString();
         $scope.reportRequest.RecsConfigurationsIds = $scope.searchParameters().Configurations.toString();
         $scope.reportRequest.RateTypeIds = $scope.searchParameters().RateTypes.toString();
         $scope.reportRequest.LicenseMethodIds = $scope.searchParameters().LicMethods.toString();
         $scope.reportRequest.AssigneeContactIds = $scope.searchParameters().Assignees.toString();
         $scope.reportRequest.LicenseTypeIds = $scope.searchParameters().LicenseTypes.toString();
         $scope.reportRequest.LicenseStatusIds = $scope.searchParameters().LicenseStatuses.toString();
         $scope.reportRequest.LicenseProductStatusReport = null;
         $scope.reportRequest.ProductPriorityReport = null;
         $scope.reportRequest.ReleaseDateBegin = $scope.searchParameters().ReleaseDateFrom;
         $scope.reportRequest.ReleaseDateEnd = $scope.searchParameters().ReleaseDateTo;
         $scope.reportRequest.SignedDateBegin = $scope.searchParameters().SignedDateFrom;
         $scope.reportRequest.SignedDateEnd = $scope.searchParameters().SignedDateTo;
         $scope.reportRequest.Notes = null;
         $scope.reportRequest.TrackTitle = null;
         $scope.reportRequest.LocalClient = null;
         $scope.reportRequest.SearchInLicenseNumber = $scope.searchParameters().IncludeLicenseNumber;
         $scope.reportRequest.FilterPriorityReport = $scope.searchParameters().FilterPriorityReport;
         $scope.reportRequest.FilterStatusReport = $scope.searchParameters().FilterStatusReport;
         $scope.reportRequest.WritersConsentTypesIds = $scope.searchParameters().WritersConsentTypes.toString();

         reportsService.addReport($scope.reportRequest).then(function (result) {
             var message = "Report Sent";
             notyService.success(message);

         }, function (error) {
             var message = "Send report failed";
